//! Parameterised command templates — Warp-style "workflows", ported from
//! anvil's `src/workflows.rs` (with forge's `src/workflows.rs` as the twin).
//!
...
//!
//! Files are loaded from `~/.config/ember/workflows/`, `$EMBER_WORKFLOW_DIR`,
//! the XDG data directories, and the development `scripts/workflows/`
//! directory. Parse failures are logged and skipped — one broken file never
//! disables the rest.
//!
...
//!
//! Every loaded or rendered command crosses the family's shared
//! `jterm_core::review_input` boundary: workflows are review-only and are
//! never auto-executed — the rendered text is filled at the prompt through the
//! same guarded path as history/block recall, and the user presses Enter.
//!
//! Deviations from the sources (all documented here per port):
//! - Directory discovery uses the `dirs` crate plus the XDG_DATA_DIRS
//!   environment fallback instead of GTK's `glib` directory functions.
//! - `welcome_notebook_path` is not ported: ember has no notebook surface.
//! - The bounded reader adds `O_NOFOLLOW` (forge's stricter variant) on top of
//!   anvil's `O_NONBLOCK | O_CLOEXEC`, so symlinked workflow files fail closed.
//! - anvil refreshes its cache on a background thread behind a single-flight
//!   gate; ember loads synchronously when the picker opens, matching the
//!   history picker's bounded-load-on-open pattern (egui is single-threaded
//!   and the read caps keep the worst case small).
...
fn read_bounded_workflow(path: &Path) -> Result<String, String> {
    ...
    #[cfg(unix)]
    {
        ...
        // Deviation: anvil uses O_NONBLOCK | O_CLOEXEC; forge additionally
        // passes O_NOFOLLOW. Ember takes the stricter set so a planted symlink
        // cannot redirect the reader outside the workflow directories.
...
/// Standard config dir: `<XDG_CONFIG_HOME>/ember/workflows/`.
pub fn user_workflow_dir() -> Option<PathBuf> {
    dirs::config_dir().map(|base| base.join("ember").join("workflows"))
}

/// System-wide XDG data directories (`XDG_DATA_DIRS`, falling back to the
/// specification default). Deviation: the sources ask GTK's `glib`; ember has
/// no GTK dependency, so this reads the environment directly.
...
fn asset_dirs_from(data_dirs: impl IntoIterator<Item = PathBuf>, kind: &str) -> Vec<PathBuf> {
    data_dirs
        .into_iter()
        .map(|base| base.join("ember").join(kind))
        .collect()
}
...
/// development. `EMBER_WORKFLOW_DIR` may add one or more platform-separated
...
pub fn workflow_dirs() -> Vec<PathBuf> {
    ...
    if let Some(extra) = std::env::var_os("EMBER_WORKFLOW_DIR") {
        dirs.extend(std::env::split_paths(&extra));
    }
    if let Some(data_dir) = dirs::data_dir() {
        dirs.push(data_dir.join("ember").join("workflows"));
    }
...
#[allow(dead_code)] // anvil exposes this tiny renderer for other surfaces; ember keeps the ported seam exercised by tests
...
mod tests {
    ...
    #[test]
    fn render_rejects_unsafe_values_and_oversized_binding_maps() {
        let w = wf("t", "echo {value}", &[("value", None)]);
        let values = HashMap::from([("value".to_string(), "safe\u{202e}txt".to_string())]);
        assert!(render(&w, &values)
            .unwrap_err()
            .contains("unsafe for review-only insertion"));
        ...
        let too_many: HashMap<String, String> = (0..MAX_WORKFLOW_ARGS + 1)
            ...
            .collect();
        assert!(render(&w, &too_many).unwrap_err().contains("more than"));
    }

    #[test]
    fn substitute_leaves_unknown_single_brace_placeholders_visible() {
        let out = substitute(
...
"#,
    ...
    #[cfg(unix)]
    #[test]
    fn workflow_symlink_is_rejected() {
        // O_NOFOLLOW (forge's stricter reader variant) must fail closed even
        // when the link target is a perfectly valid workflow file. The target
        // lives outside the scanned directory so only the link is visible.
        let dir = tempdir();
        let outside = tempdir();
        let target = outside.join("target.yaml");
        std::fs::write(&target, "name: T\ncommand: echo t\n").unwrap();
    ...
    fn installed_assets_follow_every_system_data_directory() {
        assert_eq!(
            ...
            [
                PathBuf::from("/usr/share/ember/workflows"),
                PathBuf::from("/app/share/ember/workflows")
            ]
        );
    }

    #[test]
    fn workflow_dirs_are_unique_and_bounded() {
        let dirs = workflow_dirs();
        let mut seen = HashSet::new();
        assert!(dirs.iter().all(|dir| seen.insert(dir)));
        assert!(dirs.len() <= MAX_WORKFLOW_DIRECTORIES);
        // The source-tree examples are always the final fallback.
        assert_eq!(
            dirs.last(),
            Some(
                &PathBuf::from(env!("CARGO_MANIFEST_DIR"))
    ...
    fn every_bundled_workflow_is_parseable_and_review_only() {
        // Mirrors forge's bundled-example contract: the shipped library must
        // never rot against the loader's validation rules.
