...
pub(crate) struct PersistedExecutionOutput {
    pub text: String,
    pub truncated: bool,
    pub total_bytes: u64,
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct PersistedExecution {
    pub id: String,
    pub seq: u64,
    pub command: String,
...
impl HistoryLoad {
    ...
    pub(crate) fn try_snapshot(&self) -> Result<Option<HistorySnapshot>, ()> {
...
pub(crate) enum HistoryRequestError {
    Full,
    Closed,
}
...
fn valid_rsh_session_id(id: &str) -> bool {
    crate::session::is_valid_rsh_session_id(id)
}
...
fn encode_event(mut event: OutputEvent) -> io::Result<Vec<u8>> {
    ...
    if encoded.len() > MAX_EVENT_LINE_BYTES {
        event.text = bounded_text(
            &event.text,
            crate::terminal::MAX_COMPLETED_COMMAND_OUTPUT_BYTES / 2,
        );
...
mod tests {
    ...
    fn only_rsh_compatible_execution_ids_are_persisted() {
        ...
        let valid = CompletedCommandOutput {
    ...
    fn control_heavy_output_stays_within_rshs_jsonl_limit() {
        let output = "\0".repeat(crate::terminal::MAX_COMPLETED_COMMAND_OUTPUT_BYTES);
