...
/// Embeddable state and egui surface for one task/worktree diff.
///
/// Call [`Self::request`] from a user action, then either call [`Self::show`]
/// for the built-in floating window or [`Self::show_contents`] inside another
/// egui surface. Both render paths poll without blocking.
#[derive(Default)]
...
impl AgentDiffPanel {
    ...
    /// Render a standalone floating diff window.
    pub fn show(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new("Agent diff")
            .open(&mut open)
            .default_size([760.0, 560.0])
            .resizable(true)
            .show(ctx, |ui| self.render_contents(ui));
        self.is_open = open;
    }

    /// Render only the panel body for embedding in a task or agent surface.
    ...
    fn render_contents(&self, ui: &mut egui::Ui) {
        ...
        if self.state.loading {
            ui.horizontal(|ui| {
                ui.spinner();
                ui.label("Loading tracked changes…");
            });
        }
        ...
        if self.state.truncated {
            ui.colored_label(
                ...
                format!(
                    "Diff or diagnostics exceeded the retained limits ({} KiB stdout, {} KiB stderr).",
                    MAX_DIFF_STDOUT_BYTES / 1024,
                    MAX_DIFF_STDERR_BYTES / 1024
                ),
            );
        }
        ...
        egui::ScrollArea::both()
            ...
            .show(ui, |ui| {
                ui.add(
                    egui::Label::new(RichText::new(&self.state.text).monospace())
                        .selectable(true)
...
fn is_trusted_system_owner(uid: u32) -> bool {
    ...
    // only when Ember itself is not running as the same uid; otherwise an
...
fn bounded_lossy_text(bytes: Vec<u8>, limit: usize) -> String {
    ...
    // invisible/bidirectional formatting before egui displays the diff. `?`
...
fn visible_diff_cwd(cwd: &Path) -> String {
...
mod tests {
    ...
    fn deadline_converges_when_exited_child_leaves_a_pipe_holding_descendant() {
        const HELPER_ENV: &str = "EMBER_DIFF_PIPE_HOLDER_HELPER";

        let mut command = Command::new(std::env::current_exe().expect("current test executable"));
        command
            .arg("--exact")
            .arg("agent::diff::tests::pipe_holder_helper_process")
            .arg("--nocapture")
            .env(HELPER_ENV, "1")
            .stdin(Stdio::null())
            .stdout(Stdio::piped())
