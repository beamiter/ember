...
pub struct PreparedTaskValidation {
    ...
    #[allow(dead_code)]
    pub(crate) pinned_cwd: crate::pty::PinnedDirectory,
}
...
mod tests {
    use super::*;
    use crate::agent::context::SemanticCommandContext;
    ...
    impl TestDirectory {
        fn new(label: &str) -> Self {
            ...
            let path = std::env::temp_dir().join(format!(
                "ember-validation-{label}-{}-{nanos:x}-{sequence:x}",
                std::process::id()
            ));
    ...
    fn validation_task(repository: PathBuf, worktree: PathBuf, cwd: PathBuf) -> AgentTask {
        AgentTask {
            ...
            branch: "ember/validation-fixture".to_string(),
    ...
    fn repository_and_worktree(label: &str) -> (TestDirectory, PathBuf, PathBuf) {
        ...
        git(&["init", "--quiet"]);
        git(&["config", "user.name", "Ember Validation Test"]);
        git(&["config", "user.email", "validation@example.invalid"]);
        fs::write(repository.join("tracked"), b"baseline\n").unwrap();
        git(&["add", "--", "tracked"]);
        git(&["commit", "--quiet", "-m", "baseline"]);

        let service = crate::agent::WorktreeService::new(root.0.join("managed")).unwrap();
        let repository = service.resolve_repository_root(&repository).unwrap();
        let managed = service
            .create(&crate::agent::CreateWorktreeRequest::new(
                &repository,
                "validation-fixture",
                "ember/validation-fixture",
                "HEAD",
            ))
            .unwrap();
    ...
    fn rejects_a_registered_worktree_whose_branch_identity_changed() {
        ...
        let output = Command::new("/usr/bin/git")
            ...
            .args(["switch", "--quiet", "-c", "ember/replacement"])
            .output()
            .unwrap();
